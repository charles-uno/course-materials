...
- In "real" code, you can use descriptive variable names and function names
- In Assembly, you have much less of that
- Registers just hold ones and zeroes
- There are many cases where a quick comment can help with context
- Audience: your friend who might take this class next year
...
- Use the name to load the *address*
- Value does not change
- Types we will use: `word` and `ascii`
- Use `.align 2` after `ascii` literals
...
- Otherwise just like a global constant
...
- If you see `r0`, it's probably a typo left over from the previous version of this class (ARM v7 vs ARM v8)
...
Register `x2` holds an *address*. Subtract `0x20` from that address, then load the *value* from there to `x3`:
```arm
ldr x3, [x2, -0x20]
```
...
Register `x5` holds an *address*. Add `0x10` to that address, then store the *value* from `x7` to there.
```arm
str x7, [x5, 0x10]
```
...
Work together to produce an automated test. 

You can copy-paste whatever you want from the slides or elsewhere

(But try to make something that you'll want to reuse next time!)
...
%
%- Developers write code then pass it to the test team
%- Testers make sure it works then pass it to the deployment team
%- A different team owns metrics and alerting for live code
...
%
%- You own your product end-to-end
%- Development, testing, deployment, monitoring, etc
%- What new problems might arise? How might we address them?
%
%

%### DevOps and Automation
%
%- Many different hats for the same person
%- We now have tooling to automate much of it
%- Dedicated engineers to make sure the tools fit together
%
