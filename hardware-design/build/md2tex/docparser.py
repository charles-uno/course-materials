...
class Paragraph(DocElement):
    ...
    def get_next_and_leftovers_inline(self, raw, **kwargs):
        for cls in [InlineCode, Bold, Italic, URL, Hyperlink, Comment, BinaryPrefix, HexPrefix]:
            if cls.matches(raw):
                try:
...
class BinaryPrefix(InlineBase):

    DIGS = "01"

    @classmethod
    def matches(cls, raw):
        return raw.startswith("0b") and len(raw) > 2 and raw[2] in cls.DIGS

    @classmethod
    def get_with_leftovers_inline(cls, raw: str, **kwargs: dict) -> tuple[DocElement, str]:
        assert cls.matches(raw)
        return cls("0b", **kwargs), raw[2:]

    def to_tex(self) -> str:
        return r"0\text{b}"


class HexPrefix(InlineBase):

    DIGS = "0123456789abcdefABCDEF"

    @classmethod
    def matches(cls, raw):
        return raw.startswith("0x") and len(raw) > 2 and raw[2] in cls.DIGS

    @classmethod
    def get_with_leftovers_inline(cls, raw: str, **kwargs: dict) -> tuple[DocElement, str]:
        assert cls.matches(raw)
        return cls(raw[:2], **kwargs), raw[2:]
