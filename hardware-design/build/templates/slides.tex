...
% for strikethrough in memory diagram
\usepackage{cancel}

% for big brackets next to memory diagram tables
...
\usepackage{multirow}
% vertical paragraph gaps instead of indentation
\usepackage{parskip}
\usepackage{multicol}
\usepackage{graphicx}
...
% background color on odd table rows for legibility
\usepackage[table]{xcolor}
\rowcolors{2}{AccentColor!10}{white}
...
\newcommand{\LogicXor}[2]{{\ensuremath{{#1 \text{\textasciicircum} #2}}}}
...
\node at (0.5, 0.25) {\textcolor{PaperColor}{CSCI 241: Hardware Design}};
\node at (0.5, 0.4) {\usebeamertemplate{title}};
\node at (0.5, 0.6) {\usebeamertemplate{author}};
% \node at (0.5, 0.75) {\usebeamertemplate{institute}};
\node at (0.85, 0.93) {\small\usebeamertemplate{date}};
\node at (0.5, 0.8) {\includegraphics[width=3cm]{images/stolaf-logo.png}};
\end{scope}
\end{tikzpicture}
}
