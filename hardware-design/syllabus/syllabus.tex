...
\begin{itemize}
	\item \textbf{Data Representation.} Representing numbers and other data using ones and zeroes. Binary arithmetic. Hexadecimal. Negative numbers using two's complement. Floating point numbers.
	\item \textbf{Logic Representation.} Boolean logic and expressions. Logic gates and logic circuits. Circuits for logic and arithmetic. Building logic gates out of transistors.
	\item \textbf{Instructions on Hardware.} Components of a computer. Memory hierarchy, caching, and locality. The steps of executing an instruction on a clock-driven machine. Pipelining, data hazards, and control hazards.
	\item \textbf{Assemly Globals.} Fundamentals of Assembly programming. Global constants and variables. Input and output to the terminal. Raspberry Pi setup. 
	\item \textbf{OS Concepts.} Processes, threads, and cores. Process life cycle. Booting the machine. Concurrency. Kernel and user modes. Traps and interrupts.
	\item \textbf{Assembly Functions.} Control flow in Assembly using function calls. Stack frames. Memory diagrams. Local variables.
	\item \textbf{Assembly Conditionals.} Control flow in Assembly using conditionals and loops.
	\item \textbf{Networking.} Client/server model. Packets. Routers. Protocols. Domain name resolution.
\end{itemize}
