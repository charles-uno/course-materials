...
\begin{frame}{CPU}
	...
	\begin{itemize}
		\item Instruction register (IR) holds the current instruction
		\item Program counter (PC) holds the address of the next instruction
		\item Clock management happens here too (more on this later)
	\end{itemize}

	Processing unit performs execution.
	\begin{itemize}
		\item ALU (arithmetic logic unit) does work
		\item General purpose registers store program data.
	\end{itemize}
...
\begin{frame}{Input and Output}
	...
	\begin{itemize}
		\item Input unit(s) load program data and instructions on the computer and initiate program execution.
		\item Output unit(s) store or receive program results.
		\item What are some examples of input/output hardware?
	\end{itemize}
...
\begin{frame}{Clock Speed}
	\begin{itemize}
		...
		\item 1 GHz = 1,000,000,000 Hz = 1,000,000,000 cycles per second
...
\begin{frame}{Wait a Sec}
	\begin{itemize}
		\item We fetch instructions from memory one at a time
		\item It takes ~100 CPU cycles to get data from memory
		\item Does that mean we spend 99\% of our time waiting??
		\item TLDR: the cache does a lot of work here
	\end{itemize}
\end{frame}
...
\begin{frame}{Pipelining}
	\begin{itemize}
		\item After the fetch stage is done for an instruction, its circuitry doesn't do anything until the next instruction starts
		\item Same for decode
		\item What if we start the next instruction as soon as the circuitry is available?
	\end{itemize}
\end{frame}

\begin{frame}{Behold the Pipeline}
	\begin{center}
...
\begin{frame}{Please explain that busy diagram}
	\begin{enumerate}
		\item Fetch instruction 1
		\item Decode instruction 1, fetch instruction 2
		\item Execute instruction 1, decode instruction 2, fetch instruction 3
		\item Write instruction 1, execute instruction 2, decode instruction 3, fetch instruction 4
		\item Done with instruction 1, write instruction 2, execute instruction 3, decode instruction 4, fetch instruction 5
		\item Etc
	\end{enumerate}
\end{frame}


\begin{frame}{Pipelining Upsides}
	\begin{itemize}
		\item Code runs faster
		\item One instruction takes 4 cycles (same as before)
		\item Two pipelined instructions take 5 cycles (previously: 8)
		\item Three pipelined instructions take 6 cycles (previously: 12)
		\item This is a pretty big improvement
	\end{itemize}
\end{frame}


\begin{frame}{Wait a second}
	\begin{itemize}
		\item Instructions live in memory
		\item It takes many CPU cycles to fetch data from memory
		\item We are fetching a new instruction for each instruction
		\item How can we capture pipelining wins if we spend so much time waiting for the instruction fetch?
	\end{itemize}
\end{frame}
...
\begin{frame}{Pipelining Downsides}
...
\begin{frame}{Hazards}
	\begin{itemize}
		\item Data hazard: we want to do two steps simultaneously, but we can't because they want to touch the same register
		\item Control hazard: we don't know what to do next because it depends on the result of the result of an instruction that's still running
	\end{itemize}
\end{frame}


\begin{frame}[fragile]{Data Hazards}

	\begin{minted}[linenos]{tasm}
	add r2, r1, r0
	add r4, r2, r3
\end{minted}
...
\begin{frame}[fragile]{Data Hazards}
	...
	\end{minted}
	\begin{itemize}
		\item Instruction 1 is writing data to r2 (write stage)
		\item Instruction 2 is reading data from r2 (execute stage)
		\item Those stages are supposed to happen at the same time
		\item Race condition. Different outcome based on which happens first
		\item Not good
		\item What can we do?
	\end{itemize}
\end{frame}
...
\begin{frame}{Mitigating Data Hazards}
	\begin{itemize}
		\item Compiler shenanigans. Sometimes \texttt{gcc} can figure out a way to reorder the instructions to avoid the hazard
		\item Operand forwarding. Pass the value right back into the CPU to avoid the dangerous register read
		\item Pipeling stall. Add a "bubble" to the pipeline. Wait until the operation is safe
	\end{itemize}
\end{frame}

\begin{frame}{Pipelining with a Bubble}
	\includegraphics[width=0.66\columnwidth]{images/pipelining-wiki-with-bubble}
\end{frame}

\begin{frame}[fragile]{Control Hazards}
	\begin{minted}[linenos]{python}
	if r1 == 0:
		...
	else:
		...
	\end{minted}
	Which instruction should we load into the pipeline after 1?
\end{frame}

\begin{frame}{Mitigating Control Hazards}
	\begin{itemize}
		\item Pipeling stall. Add a "bubble" to the pipeline. Wait until we know the result of the conditional
		\item Branch prediction. Guess. If we guess wrong, go back and do it right
		\item Eager execution. Try them both. Once we get the result, throw away the moot one
	\end{itemize}
\end{frame}
