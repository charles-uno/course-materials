...
% nasm - dislikes comments with @
	\begin{minted}{nasm}
	@ global read-only data (aka constants)
	.section .rodata
	...
	.global main
	main:
	@ load the string address to r0
	ldr r0, =greeting
	@ print the string from r0
	bl printf
...
\begin{frame}{Why Learn Assembly?}
	...
	in C, Java, Python, etc, there's a lot of work happening under the hood
	objects, abstraction, variable scope
	the language does a lot of work for you
	makes the code more legible
	lets you worry less about what's happening in the hardware

	but we want to worry about the hardware!
	each line of assembly code is one CPU instruction (more or less)
	each type of processor has its own assembly dialect. the code you write on a pi will not work on another machine

	helps us understand much more precisely what's happening
\end{frame}


\begin{frame}{Memory and Registers}

	Memory is your bookshelf.

	Register is your desk.

	If you want to read or write something, you have to take the notebook off your shelf and put it on the desk.

	When coding in Assembly, we have to explicitly move data between memory and registers
