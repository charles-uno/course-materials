...
\begin{frame}{FDEW}
	...
	\begin{itemize}
		...
		\item Execute does work. The rest is overhead!
	\end{itemize}
...
Fetch the instruction we are about to execute and bring it to the CPU
	\begin{itemize}
		\item Special register pc (program counter) stores the address of the instruction we will execute this cycle
		\item Read that instruction from memory. Uses address bus, control bus
		\item Store the instruction to special register ir (instruction register). Uses data bus
...
Decode the instruction so we can execute it
	\begin{itemize}
		\item Special register pc (program counter) stores the address of the instruction we will execute this cycle
		\item Read that instruction from memory. Uses address bus, control bus
		\item Store the instruction to special register ir (instruction register). Uses data bus
