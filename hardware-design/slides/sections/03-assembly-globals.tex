...
\begin{frame}{Coding in Assembly}
	\begin{itemize}
		\item We'll be doing some coding exercises in assembly on emulators and on a Raspberry Pi
...
\begin{frame}{Calling Printf}
	...
	\begin{itemize}
		...
		\item if the output string includes another formst identifier, it grabs the value from \Arm{x2}, etc
	\end{itemize}
\end{frame}
...
\begin{frame}{Loading Values from Memory}
	\begin{itemize}
		...
		\item \Arm{ldr x3, =n_thumbs} loads the address of global variable \Arm{n_thumbs} into \Arm{x3}
		\item \Arm{ldr x1, [x3]} goes to the address at \Arm{x3}, loads the value, and stores it in \Arm{x1}
